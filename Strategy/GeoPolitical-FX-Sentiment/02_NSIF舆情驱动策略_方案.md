...
## 九、v2.0 市场机制修订：通胀传导路径感知（2026-04-13）

> **修订背景**：2026 年 4 月 13 日市场出现关键异常——VIX 单日涨幅 +10.19%（强风险规避），但黄金同日下跌 0.91%（$4,748 → $4,744）；原油（CL=F）涨幅 +6.30%。这与 NSIF v1.0 中"地缘事件 → 黄金看多"的默认逻辑完全相反。根本原因是**市场机制发生了机制性切换**：当前主导叙事是"霍尔木兹封锁→油价→输入性通胀→美联储加息预期升温→实际利率上行→压制黄金"，而非传统的"战争→避险买金"逻辑。

### 9.1 双路径框架：地缘避险 vs. 通胀传导

NSIF v1.0 假设"战争新闻 = 黄金看多信号"。这在以下情景成立：
- 直接核威胁、领导人被暗杀、关键军事突破等**恐慌事件**
- 市场关注点是"生存风险"而非"通胀"

但在以下情景，逻辑**完全逆转**：
- 供应链中断（霍尔木兹封锁）→ 油价暴涨 → 通胀预期飙升 → 加息预期主导 → 黄金下跌
- 这是 2022 年俄乌战争后期的翻版（油价涨 → Fed 加息 → 黄金跑输原油）

**两种机制的资产响应对比**：

| 新闻类型 | 黄金信号 | 原油信号 | USD 信号 | CAD 信号 |
|---------|---------|---------|---------|---------|
| 军事升级（伤亡/核）| **+（避险）** | + | ±（分裂）| - |
| 供应中断（霍尔木兹）| **−（通胀→加息）** | **++** | **+（Fed 鹰派）** | **++（商品）** |
| 停火谈判 | − | − | + | − |
| 通胀/CPI 数据 | **−（加息确认）** | ± | + | + |
| 经济衰退信号 | +（降息预期）| −− | − | − |

### 9.2 NSIF v2.0：加入机制识别层（Regime Layer）

在原有信号生成流程前，插入**机制识别器（Regime Classifier）**：

```
路透社新闻（每条）
      ↓
  【Regime Classifier】（LLM 分类）
      │
      ├─ regime = "GEOPOL_PANIC"（军事威胁/人员伤亡/核威胁）
      │       → 传统 NSIF 计算，黄金 geo_amplifier 生效
      │
      ├─ regime = "OIL_INFLATION"（供应中断/霍尔木兹/油价/通胀/加息）
      │       → 黄金方向取反（NSIF_gold = −原值 × inflation_weight）
      │       → 原油/CAD NSIF 放大（× 1.5）
      │       → USD 方向取正
      │
      ├─ regime = "DEESCALATION"（停火/谈判/外交突破）
      │       → 黄金/CHF 看空，原油看空，风险货币回升
      │
      └─ regime = "RECESSION_FEAR"（需求破坏/经济崩溃/萧条）
              → 黄金重新看多（降息预期），原油看空
```

**LLM Regime 分类提示词核心逻辑**：
```
对每条新闻，判断其主要传导路径：
A. 直接安全威胁（军事行动、核威胁、人员伤亡）→ GEOPOL_PANIC
B. 供应/能源/通胀（Hormuz、油价、封锁、CPI、rate hike）→ OIL_INFLATION
C. 外交/停火（ceasefire、talks、agreement、deal）→ DEESCALATION
D. 经济/衰退（recession、GDP、demand destruction、layoffs）→ RECESSION_FEAR
E. 央行/货币政策（Fed hike/cut、BoJ、ECB）→ 单独处理（见 9.3）
```

### 9.3 加息恐惧指数（RHFI：Rate Hike Fear Index）

新增独立的**RHFI 计算模块**，从 Reuters 新闻中实时提取加息恐惧强度：

```python
RHFI_t = Σ (news_i.rate_hike_signal × news_i.source_weight × λ_rate(t - t_i))

# 触发关键词：
# 正向（加息恐惧）：rate hike, CPI surprise, hawkish, tightening,
#                   inflation spike, oil-driven inflation
# 负向（降息预期）：rate cut, dovish, recession, demand destruction

# 时间衰减半衰期：λ_rate = 4h（利率预期变化较快）
```
...
| RHFI 水平 | 含义 | 黄金信号修正 |
|----------|------|------------|
| RHFI < 0.2 | 降息预期 / 中性 | 黄金允许看多（传统 NSIF 生效）|
| 0.2 ≤ RHFI < 0.5 | 温和加息担忧 | 黄金 NSIF 折扣 50% |
| RHFI ≥ 0.5 | 强烈加息恐惧 | 黄金 NSIF 翻转（正 → 负）或禁止开多 |

**当前状态（2026-04-13）**：RHFI 估计 ≥ 0.6（霍尔木兹封锁已将油价推至 $100+，通胀预期显著上升），黄金信号处于**翻转区间**。

### 9.4 v2.0 新增核心交易逻辑

#### 策略 ①：油价-CAD 高相关追踪（新增）

当 `regime = "OIL_INFLATION"` 时：
- NSIF 生成油价正向信号 → **USD/CAD 看空**（CAD 受益于油价）
- 持仓窗口延长至 2-8 小时（油价影响 CAD 的滞后通常为 15-60 分钟）
- 止损：ATR × 1.5（油价波动率更高）

#### 策略 ②：通胀路径做空黄金（新增）

当 RHFI ≥ 0.5 且 `regime = "OIL_INFLATION"` 时：
- **新增 XAU/USD 做空触发器**：在油价新闻冲击后（+3~10 分钟），黄金因惯性先短暂上涨，随后在加息预期压制下回落
- 交易策略：等黄金在油价新闻后冲高至 NSIF 阻力位，**反向做空**
- 止损：当 regime 切换至 GEOPOL_PANIC 时立即平空

#### 策略 ③：黄金机制切换预警（新增）

RHFI 从 0.5+ 快速下降至 0.2 以下（e.g., 停火协议 → 油价暴跌 → 通胀预期消退）：
- **黄金将经历机制切换**：从"加息压制"转回"避险买入"
- 在 RHFI 下穿 0.3 时，**提前布局黄金多单**（机制切换早于价格反应）
- 这是一个高 alpha 的跨机制套利窗口

### 9.5 参数更新

| 参数 | v1.0 默认值 | v2.0 修订值 | 说明 |
|------|------------|------------|------|
| `geo_amplifier` | 1.0-2.0 | 仅在 GEOPOL_PANIC 下生效 | OIL_INFLATION 机制下设为 0.3 |
| `gold_regime_flip` | 不存在 | True（RHFI≥0.5 时）| 新增参数 |
| `oil_cad_amplifier` | 不存在 | 1.5 | OIL_INFLATION 机制下 CAD 放大 |
| `rhfi_threshold_high` | 不存在 | 0.5 | 黄金信号翻转阈值 |
| `rhfi_threshold_low` | 不存在 | 0.2 | 降息预期域，传统逻辑恢复 |
| `inflation_news_halflife` | 不适用 | 4h | RHFI 时间衰减参数 |
